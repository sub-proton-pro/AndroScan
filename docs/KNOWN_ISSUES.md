...
### ISSUE-022: Phase 13 v2.x flowchart layout has no call-hierarchy indentation — diagnosis sharpened 2026-05-20 by DEC-031 v3-preview empirical evidence: slicer-side data-model gap, NOT just missing call-graph edges
- status: **Resolved (Phase 13 v3.X-next.2, 2026-05-21)** — see § 8 Resolved issues below for the closing record. Original framing preserved verbatim here for the as-built diagnosis history.
- ~~status: Open (Phase 13 v2.1 candidate; operator-demand-gated; **opened 2026-05-19 by DEC-030 ratification**; **diagnosis sharpened 2026-05-20 by DEC-031 Proposed — see § Diagnosis update**)~~
- impact: Medium
- area: Behavior Trace v3 / `ExecutionFlow` layout / slicer-side data model
- introduced / observed: 2026-05-19 (Phase 13 v2.0 ship; one of the four friction signals operator dogfooding 2026-05-08 → 2026-05-19 surfaced on the v1 ship). Diagnosis sharpened 2026-05-20 by v3 preview dogfood (`?flow=v3` URL gate, commit `c59d879`) which confirmed call-graph injection alone is insufficient.
- summary:
  Phase 13 v1 introduced the column-rank flowchart layout in 13.6 (BFS from the entry method along decision edges → assigns each node a `layoutRank` integer → renderer groups nodes by rank into vertical columns). Without call-graph caller→callee edges in the FE graph builder, methods that don't appear as decision targets (decision SOURCES whose decisions all flow to other-method targets; bypass-plan targets whose source method isn't itself reachable from entry via decision edges) fall through `ranks.get(succ) ?? 0` to rank 0 alongside the entry node, producing a "flat" layout with no clear call hierarchy. Phase 13 v2.0 (DEC-030 Q5 = (b)) added a within-column entry-first sort which fixes the v1 "boxes above Entry method" complaint at rank 0, but does NOT add call-graph edges and therefore does NOT give the flowchart real call-hierarchy indentation. v2.0 left this as a separate v2.1 sub-step, **operator-demand-gated** on dogfood evidence that the v2.0 column-rank layout still feels flat in real-app use. **2026-05-19 v2.0.1 closing-note dogfood signal (a) "all three" answer FIRED the v2.1 promotion trigger.** Operator then dogfooded the **v3 preview** (`?flow=v3` URL gate; top-down Dagre layout, dropping synthetic ALLOW/DENY/neutral sinks, verdict-summary chip on every gate, `FRAMEWORK_CLASS_PREFIXES` denylist replacing v3.0's candidate-gate-only filter) on the same WeakBank `MainActivity.onCreate$lambda$0` anchor 2026-05-20 (commit `c59d879`) — the v3 preview was specifically constructed to test the originally-scoped v2.1 fix (call-graph injection territory). **Result: the flat-layout friction persists.** Three surviving app-code methods (`LabStore.getPin`, `MainActivity.renderState`, `MainActivity.validatePin`) still sit horizontally at rank 1 below the entry on the anchor. Walking through the data flow surfaced the deeper root cause that this issue now tracks.
- why it matters:
...
- diagnosis update (2026-05-20 via DEC-031 Proposed):
  The originally-scoped fix (inject `call_graph.sqlite::neighbors()` as `kind: "call"` edges before BFS rank computation) addresses one of two compounding limits but not both. v3 preview demonstrated:
    1. **Synthesised `entry → DecisionPoint.method` `call` edges** (one per gate-bearing method, fanning out from the entry) plus **plan-target edges** (only emitted for `allow` verdicts) are the ONLY edges v3.1 has to give Dagre depth signal. On anchors with `0 plans` (e.g., WeakBank `lambda$0`: 9 decisions / 0 plans / all neutral), no plan-target edges fire and all decision-bearing callees collapse to rank 1 under Dagre's longest-path layering — three siblings at the same rank.
    2. **Call-graph injection (the originally-scoped v2.1 fix) would give a wider fan-out**, not a vertical chain. If the entry's body sequentially calls `check_active_session`, `check_input`, `validate_pin`, `create_session`, `PostLoginActivity`, `Login_UI_text`, the static call graph treats all six as direct callees of the entry at call-graph depth = 1; Dagre lays them out as six rank-1 siblings, NOT as a six-rank vertical chain.
  The operator's mental model — confirmed verbatim 2026-05-20 — is "the ranking is simply done based on presence within a method (even when it is the entry). The idea of a rank is to put them in the same column of indentation, not lay them horizontally. And the order in which they appear in login's definition should decide how far down from login in the graph they are." This is a **CFG-position-aware ranking** keyed on statement order inside the entry's body, NOT a call-tree-depth ranking. Achieving it requires:
    - **Per-method invocation sequence** with instruction indices for ordering.
    - **Per-invocation branch-arm label** (which `if`-arm of which decision does this invoke live in) so off-axis terminals (deny/Ret: False branches) become siblings of the rank-below sequential call rather than collapsing into a verdict chip.
    - **Decision-free callees kept as first-class nodes** (e.g., `validate_pin` calling `validate_pin_final`, `create_session` doing UUID gen) — v3.1's filter rule (`isEntry || isDecisionSource || isPlanTarget`) drops them, but the operator's hand-mockup keeps them as call points.
  None of this data lives in `BehaviorAnchor` today. The slicer pass computes basic-block dominance + invoke instructions during the predicate-origin walk and discards them before producing the anchor. **The fix is slicer-side first, renderer-side second.** Call-graph injection alone (ISSUE-022's original framing) becomes a SUBSET of the broader fix: useful as a layout aid for the wide-fan-out case where decision-free callees appear between two decisions, but not sufficient on its own.
- current workaround:
  Operators can mentally reconstruct the call hierarchy by clicking through nodes in the Inspector pane and reading the source-line + class+method context. The `<Inspector>`'s "Predicate origin" + "Bypass plans" sections also surface caller-callee relationships textually when they're load-bearing for the operator's task. For complex anchors, narrowing the entry method or reducing the `hops` parameter scopes the closure tighter so the flat layout has fewer competing rank-0 nodes. Operators can also URL-toggle `?flow=v3` to dogfood the v3 preview's cosmetic improvements (framework-package denylist + verdict-summary chip on every gate + no synthetic ALLOW/DENY sinks) while v4 ramps via DEC-031 ratification.
- recommended fix (DEC-031 Proposed; awaiting Q1-Q8 ratification):
  Two-layer change — additive at the schema layer, new emitter at the renderer layer.
    1. **Slicer side** (`androscan/analysis/`): extend `BehaviorAnchor` with a new `method_invocations: dict[str, tuple[CallSite, ...]]` field where `CallSite = (caller, instruction_index, callee, in_branch_of, branch_label)`. Additive schema (Q4 = (b) recommendation: keep `SCHEMA_VERSION` at 2 with default empty dict so v2 / v2.0.1 caches keep deserialising; v4 emitter falls back to v3.x layout when field is empty). Implementation locus extends the existing slicer pass in `androscan/analysis/slicing.py` (Q3 = (a) recommendation: same Soot CFG visit that already computes basic-block dominance for the predicate-origin slicer). Soot already has the data; the slicer just stops discarding it post-walk.
    2. **Renderer side** (`androscan/web/frontend/src/components/trace/`): new `executionFlowGraphV4.ts` (or evolved-in-place per Q7 = (a) recommendation: keep `?flow=v3` as rollback target, ship v4 behind `?flow=v4`) that consumes `BehaviorAnchor.method_invocations` to emit sequential `prev_invoke → next_invoke` `call` edges between consecutive callees in the same branch arm, with sibling subtrees per branch of each decision. Dagre still does the layering (`rankdir: 'TB'`, `network-simplex` ranker); the new emitter changes only **which edges Dagre sees**.
  Call-graph injection (the original v2.1 fix per DEC-030) is naturally subsumed — `method_invocations` is effectively a per-anchor subset of the static call graph annotated with CFG-position data. The new field includes everything ISSUE-022's original framing required PLUS the position data; no separate call-graph route needed.
- v2.1 promotion trigger:
  Subjective / dogfood-based per DEC-030 v2.1 trigger = (a). **Already FIRED 2026-05-19** by operator's v2.0.1 closing-note (d) "all three" answer; **further sharpened 2026-05-20** by v3 preview dogfood confirming the originally-scoped call-graph-injection fix is necessary but not sufficient. v2.1 ratification surface is DEC-031 (Proposed; awaits Q1-Q8 operator answers in v2-next.0 Ask-mode planning checkpoint).
- related tasks:
  - `docs/TASKS.md` § Active Task (the v2-next planning-checkpoint block tracks this issue's resolution under DEC-031)
- related docs:
  - `docs/DECISIONS.md` DEC-030 (locks the deferral; documents the v2.1 gating trigger; locks the call-edge styling as "thin gray, no arrowhead, no label" — those styling locks carry over to v4's `kind: "call"` edges)
  - `docs/DECISIONS.md` DEC-031 (Proposed; the ratification surface that proposes the slicer-side `method_invocations` extension + the new v4 emitter; Q1-Q8 ratification pending; covers the broader scope including signals (b) + (c) from v2.0.1 closing-note dogfood)
  - `androscan/analysis/call_graph.py::neighbors()` (the BE walker the original v2.1 fix would have consumed — still relevant for the wide-fan-out case but no longer the primary fix; DEC-031's `method_invocations` field carries equivalent + position data)
  - `androscan/analysis/slicing.py` (where DEC-031's slicer-side extension would land; same Soot pass already computes basic-block dominance for predicate-origin walking)
  - `androscan/analysis/trace_types.py::BehaviorAnchor` (the schema that gets the new `method_invocations` field additively)
  - `androscan/web/frontend/src/components/trace/executionFlowGraphV3.ts` (the v3 preview that empirically surfaced the data-model gap; commit `c59d879`)
  - `androscan/web/frontend/src/components/trace/executionFlowGraphV4.ts` (the proposed new emitter; doesn't exist yet — DEC-031 ratification gates creation)

---

### ISSUE-023: Workbench chat SSE endpoint hard-codes the Ollama codepath; llama.cpp operators see `Cannot connect to Ollama at http://localhost:11434` errors even when their provider is correctly configured + healthy
- status: Open (LCP.8 candidate; **opened 2026-05-21** by operator dogfood under llama.cpp on the Reports tab chat dock; explicit regression carried over from the LCP track close (2026-05-06))
- impact: High for any operator on `llm.provider: "llamacpp"` (the chat dock is the operator's primary surface for chat / agentic-loop interaction with skills; this bug makes it completely unusable for llama.cpp operators); zero impact on operators staying on `llm.provider: "ollama"` (which is still the loader default in shipped `global_config.yaml`)
- area: LLM client / chat SSE / provider routing
- introduced / observed: **Regression introduced 2026-05-06 at LCP.2** (commit `fc33658` — `_call_llamacpp()` HTTP path + router branch in `complete()`). Observed 2026-05-21 by operator after rebuilding their `global_config.yaml` to `llm.provider: "llamacpp"` while sanity-checking the v3.X-next.2 ship. Surfaced when chat-dock SSE request returned `[chat failed: LLM call failed: RuntimeError: Cannot connect to Ollama at http://localhost:11434.` despite the Settings → Status panel showing the llama.cpp LLM card green at 67ms with the configured GGUF loaded.
- summary:
  LCP.2 introduced a provider-aware router in `androscan/llm/client.py::complete()` (line 1056-1106) that calls `config.provider_kind()` and dispatches to one of `_call_ollama` / `_call_llamacpp` / `_call_cloud`. **However, the parallel `stream_complete()` helper (line 295-355) was not made provider-aware** — it stays hard-coded to the Ollama `/api/chat` endpoint reading `config.ollama_base_url` / `config.ollama_model` directly, with no router on `provider_kind()`. The docstring at line 312-314 explicitly admits the design carve-out: *"Cloud providers don't currently flow through this helper; the workbench SSE endpoint is Ollama-only by design (matches the streaming/thinking semantics of the local model)."* That carve-out was accurate pre-LCP (only alternatives at that point were *cloud* providers with different streaming semantics), but the moment LCP shipped llama.cpp as a second LOCAL provider, the carve-out became wrong — llama.cpp's `/v1/chat/completions` endpoint supports OpenAI-style SSE streaming on the same shim that `_call_llamacpp()` already uses for the non-streaming path.
  The chat dock SSE flow (`androscan/web/chat.py:631-634`) imports `stream_complete` as the default `streamer` and passes it the operator's Config. When `Config.provider_kind() == "local-openai-compat"`, `stream_complete` still attempts to POST to whatever `config.ollama_base_url` resolves to (defaulting to `http://localhost:11434`) — operators get the misleading "Cannot connect to Ollama" error message even though Ollama is irrelevant to their setup.
- why it matters:
  The chat dock is the operator's primary surface for chat / agentic-loop interaction with skills + the only consumer of the `summarise_method` / `suggest_trace_entry` / `MethodSummaryWidget` / `TraceEntryCandidateWidget` chat-widget renderer (Phase 11 v2.1.5 + Phase 13 v1 13.9). For an operator on llama.cpp this bug breaks: Ask AI tier-2 path on the Trace entry-method form (Phase 11 v2.1.4); "Discuss in chat" affordance on `<Inspector>` (Phase 13 v1 13.9); cross-tab chat affordances on Reports + Inspect findings (Phase 6+); the `<TraceEntryCandidateWidget>` agentic-loop substrate (Phase 11 v2.1.5). The Status panel's LLM card has no chat-completion round-trip exercise (it only does `GET /health` + `GET /v1/models` per `androscan/web/health_probes.py::probe_llamacpp`) so the bug is completely invisible to operators relying on Status as a smoke test before chat. The error message is also misleading — it points operators at Ollama when their config is correct.
- current workaround:
  Switch `llm.provider:` in `global_config.yaml` back to `"ollama"` AND have a working Ollama server running on `http://localhost:11434` for chat-only traffic — but this defeats the entire LCP track for operators who chose llama.cpp specifically to escape Ollama's 36 GB memory pressure on M-series hardware (operator's 2026-05-05 status share). Non-streaming paths (every LLM-tier skill: `trace_behavior`, `summarise_method`, `suggest_trace_entry`, `generate_frida_hook`, `query_call_graph`, `search_decompiled_sources`, etc.) all flow through `complete()` and route correctly to llama.cpp, so static analysis + skill-based widgets keep working — only the chat dock SSE path is broken.
- recommended fix (LCP.8 candidate):
  Add a provider-aware router to `stream_complete()` mirroring `complete()`'s shape. Two implementation options to ratify at the LCP.8.0 planning checkpoint:
    1. **Add a sibling `_stream_llamacpp()`** that hits `POST {base_url}/v1/chat/completions` with `stream: true`, parses the OpenAI-style `data: ` SSE frames in the response stream, and surfaces `on_token` / `on_thinking` from each chunk's `delta.content` field; wrap `stream_complete()` in a `provider_kind()`-aware router with the same three-way switch shape `complete()` uses (`"local-ollama"` → existing Ollama path; `"local-openai-compat"` → new llama.cpp path; `"cloud"` → cloud streaming path or fallback to non-streaming). Estimated ~+150 LOC client + ~+15 tests in `tests/test_llm_client.py` covering the new path's request-body shape + SSE chunk parsing + `<think>` strip parity + transient HTTP retry. **Recommended option** — preserves streaming UX on llama.cpp, llama.cpp's OpenAI-compat shim supports streaming natively, parsing is well-documented (the existing cloud-provider path may already have OpenAI-compat SSE parsing to reuse).
    2. **Fall back to non-streaming `complete()` when `provider_kind() == "local-openai-compat"`** — simpler ~30 LOC change in `androscan/web/chat.py` that picks the streamer based on provider kind. Loses streaming UX on llama.cpp (operator sees the full reply land at once instead of token-by-token), which is a meaningful UX regression for long chat replies + the agentic-loop substrate. Avoid unless option (1)'s SSE parsing turns out unexpectedly difficult to wire.
  Either way: extend `androscan/web/health_probes.py::probe_llamacpp` to optionally exercise a single tiny chat-completion round-trip (~3-token "ping" → "pong" call) so the Status card surfaces chat-path liveness rather than just liveness + model-list. Open question at LCP.8.0 whether that probe extension is part of LCP.8 or a separate sub-step.
- promotion trigger:
  Not gated on dogfood evidence — this is a real regression with a clear repro (operator with `llm.provider: "llamacpp"` opens chat dock, sends any message, gets `Cannot connect to Ollama` error). Promote LCP.8 to Active when operator schedules the planning checkpoint.
- related tasks:
  - `docs/TASKS.md` § LCP — llama.cpp local provider — sub-step backlog (LCP.8 row, candidate stub)
- related docs:
  - `docs/DECISIONS.md` DEC-027 (LCP track ratification; LCP.2 sub-step shipped the `complete()` provider router but not the `stream_complete()` one)
  - `androscan/llm/client.py::stream_complete` (line 295-355; the hard-coded Ollama helper)
  - `androscan/llm/client.py::complete` (line 1056-1106; the provider-aware router that should be mirrored)
  - `androscan/web/chat.py::_chat_sse_endpoint` (line 631-634; the consumer that imports `stream_complete` as the default `streamer`)
  - `androscan/web/health_probes.py::probe_llamacpp` (line 862; the liveness probe that doesn't exercise the chat path)

---

_(See § 8 Resolved issues — `ISSUE-016` was closed by **LCP.6 (2026-05-06)**: GBNF grammar enforcement for llama.cpp + JSON-schema mode for Ollama 0.5.0+ now constrain the response envelope at the model's logits-sampling level. The fail-soft retry path in `androscan/internal/workflow.py` is still in place but is no longer the primary defense; operators on Q4_K_M / IQ4_XS quants regain reliable structured-JSON output. `ISSUE-022` was closed by **Phase 13 v3.X-next.2 (2026-05-21)**: the slicer-side `method_invocations` data extension (v3.X-next.1) + CFG-aware emitter (v3.X-next.2) produce vertical chains keyed on instruction-index order from inside the entry method's body, exactly matching the operator's mental model recorded verbatim in DEC-031.)_
...
### ISSUE-022: Phase 13 v2.x flowchart layout has no call-hierarchy indentation — closed by slicer-side `method_invocations` data extension + CFG-aware emitter (Phase 13 v3.X-next track)
- status: Resolved
- resolved date: 2026-05-21 (Phase 13 v3.X-next.2 — Behavior Trace V3 promoted to production; closes the slicer-side data-model gap DEC-031 diagnosed)
- resolution summary:
  Two-step landing across two same-calendar-day commits per DEC-031 Q1=(b′) split-under-path-B. **v3.X-next.1 (2026-05-21 — backend-only slicer extension):** new `CallSite` frozen dataclass in `androscan/analysis/trace_types.py` (5 fields — `caller`, `instruction_index`, `callee`, `in_branch_of`, `branch_label`) + new `method_invocations: dict[str, tuple[CallSite, ...]] = field(default_factory=dict)` field on `BehaviorAnchor`; new `extract_call_sites(...)` public helper in `androscan/analysis/slicing.py` that walks the instruction stream via the existing `_RE_INVOKE` regex, applies a `FRAMEWORK_CLASS_PREFIXES` denylist (10-entry list 1:1-mirrored against the FE v3.1 emitter's), and emits dominator-annotated CallSites in `instruction_index` order; `_compute_branch_dominance` helper computes the per-decision arm-start boundary heuristic so `in_branch_of` + `branch_label` ride with each CallSite. Wire-compatible additive at schema v2 per Q4=(b) — legacy `trace.sqlite` caches deserialise with empty `method_invocations` via `androscan/internal/trace_cache.py`'s tolerant `_decode_method_invocations` helper. **v3.X-next.2 (2026-05-21 — frontend-only emitter + closing beat):** `executionFlowGraphV3.ts` evolved in-place to consume `method_invocations` via new `emitInvokeEdges()` helper that walks per-caller `CallSite[]` in `instruction_index` order, partitions calls per `(in_branch_of, branch_label)` arm into independent chains, suppresses self-loop edges while advancing the chain, returns the set of caller node ids that emitted invokes. Phase-3 edge emission calls `emitInvokeEdges` first as the primary CFG-aware channel, then falls back to the v3.1 synthesised `entry → decision-source ``call`` ` edges (Q5=(a) gap-fallback) ONLY when the entry's own invocations list is empty / missing. New `isInvokeTarget` provenance flag introduced to fix a bug surfaced by unit tests where tagging invoke-only callees as `isPlanTarget` had incorrectly bypassed the `gatesOnly` framework filter + false-flagged invoke targets as `possiblyInlined`. New `"invoke"` edge variant + `instructionIndex` / `inBranchOf` / `branchLabel` fields. `?flow=v3` URL gate removed per DEC-031 Q7=(i); v2-pair `executionFlowGraph.ts` + `ExecutionFlow.tsx` deleted per N2=(i) immediate-deletion (operator-confirmed pre-live tool, no straddle commit / migration window / rollback affordance). 29 new FE unit tests via vitest covering pure helpers, empty anchors, verdict routing, `hideRetPills`, `gatesOnly` framework filter, invoke chaining, branch-arm forking, framework-filtered callee gap, Q5=(a) gap-fallback, and `graphV3Stats` (all 29 pass). Operator dogfood on the same WeakBank `MainActivity.onCreate$lambda$0` anchor (the v2.0.1 + v3-preview-+-DEC-031 trigger anchor) verified the new layout: `validatePin` is the ONLY direct app-code invoke of `onCreate$lambda$0` (per smali line 135), so it correctly anchors as the sole rank-1 child of the entry, with `getPin` / `setLoggedIn` / `renderState` / `startActivity` cascading down through `validatePin`'s body (smali lines 526 / 568 / 581 / 643) — matching the operator's mental model recorded verbatim in DEC-031 ("the order in which they appear in login's definition should decide how far down from login in the graph they are"). The pre-fix flat layout had hoisted all decision-bearing methods in the closure to rank 1 as siblings of the entry, conflating "in the closure" with "directly called by entry"; the post-fix CFG-aware layout uses bytecode-order invoke instructions as the source of truth.
- related tasks/docs:
  - `docs/TASKS.md` § **Phase 13 v3.X-next — CFG-aware Behavior Trace flowchart ranking — sub-step backlog** rows v3.X-next.0 → v3.X-next.2 (all marked complete 2026-05-21).
  - `docs/DECISIONS.md` **DEC-031** (Active; full Q&A record + adjacent UX/architecture locks N1-N9 + post-ratification migration story + v3.X-next track-close closing note).
  - `docs/STATE.md` (v3.X-next.0 / v3.X-next.1.0 / v3.X-next.1 / v3.X-next.2.0 / v3.X-next.2 entries under "Recent completed work" + "Next expected milestone" flipped to operator's call between LCP.7 / Phase 12 / v3.X-next.4 / v3.X-next.3).
  - `androscan/analysis/trace_types.py::CallSite` + `BehaviorAnchor.method_invocations` (the new wire-shape primitives).
  - `androscan/analysis/slicing.py::extract_call_sites` + `_compute_branch_dominance` + `FRAMEWORK_CLASS_PREFIXES` (the slicer-side extension).
  - `androscan/web/frontend/src/components/trace/executionFlowGraphV3.ts::emitInvokeEdges` (the FE CFG-aware emitter).
  - `androscan/web/frontend/src/components/trace/executionFlowGraphV3.test.ts` (29 FE unit tests pinning the new emitter behavior).
  - `androscan/web/frontend/src/components/trace/ExecutionFlow.tsx` + `executionFlowGraph.ts` (DELETED at v3.X-next.2 per N2=(i)).

### ISSUE-024: Settings → Status panel reports RAG embed provider as healthy (green dot) when the configured fastembed model can't actually load — operator-visible false positive
- status: Resolved
- resolved date: 2026-05-25 (single-commit bug fix; opened + closed in the same commit per the d9c0dc7 cadence for surface-of-a-day bugs that don't need a multi-step planning checkpoint)
- impact: Medium — operator-visible UX lie. Status panel said GREEN while every Inspect-tab UI mapping query failed with `embed provider unavailable: fastembed failed to load model 'BAAI/bge-small-en-v1.5': NO_SUCHFILE`. Real correctness signal masked by the probe.
- area: web / health probes / RAG status surface
- introduced / observed: **2026-05-22** during operator dogfood on `com_example_weakbank_low`. The bug has been present since `probe_embed_provider` was introduced (the original fastembed branch's `ok = bool(avail.get("ok"))` only checked package importability), but only surfaced when the operator's local fastembed cache layout diverged from the probe's heuristic guess.
- summary:
  Three-layer bug. **Layer 1 (BE truthfulness)** — `probe_embed_provider`'s fastembed branch set `ok = bool(avail.get("ok"))` — i.e. ok if the Python package was importable, ignoring whether the configured model was actually loadable. **Layer 2 (FE rendering)** — the Settings tab's `<StatusCardView card={globalStatus.rag_provider} />` call site at `SettingsTab.tsx:1035` passed no `extras`, leaving the card body blank even when the probe payload was fully populated. Operator couldn't tell which model was being probed, what the cache state was, or how long the probe took. **Layer 3 (per-app cross-reference)** — `_rag_card` in `status_routes.py` reported the per-app `RAG index` as ready based purely on on-disk file existence, never cross-referencing whether the global embed provider could actually serve queries. So even when Layer 1 was fixed, the per-app card would still lie about query viability.
- resolution summary:
  Single bug-fix commit, no DEC, no Q-matrix, no track designation (cadence matches the operator's pushback that "it is just a bug"; pre-fix scoping had over-engineered this as a `DEC-032 RAG.x` parallel infrastructure track with 8-question planning checkpoint — that scope was rejected in favor of right-sized bug-fix cadence). **As-built shape, ~+325 LOC across 5 files:**
  - **`androscan/web/health_probes.py`** (+~150 LOC) — warm-singleton functional probe replaces the heuristic-cache check. New `_probe_fastembed_warm(model)` instantiates `FastEmbedProvider` (whose constructor at `embed.py:108-139` already runs ONNX load + dim probe — successful instantiation IS the functional probe). Module-level cache `_warm_embed_probe_cache: dict[(provider, model), result_dict]` + `threading.Lock` for thread safety under `asyncio.to_thread`. Cold-start pays 0-5s; warm path is sub-µs dict lookup. Failures are cached too so a doomed model load doesn't re-fire on every 15s status poll. New `last_known_embed_provider_ok() -> Optional[bool]` cheap-sync helper exposes the cached state to other modules without firing a probe. New `_reset_warm_embed_probe_cache_for_tests()` hook so pytest can isolate cases.
  - **`androscan/web/status_routes.py`** (+~20 LOC) — `_rag_card` gains `embed_provider_ok: Optional[bool]` kwarg; when `embed_provider_ok is False` AND `on_disk_ready`, the card downgrades from green to red+hint (`"queries unavailable — embed provider is down (see Global → RAG embed provider card)"`). `embed_provider_ok=None` (probe hasn't fired yet) leaves the card unchanged — avoids false-negativing on first per-app load. `_gather_per_app` wires `last_known_embed_provider_ok()` into the call.
  - **`androscan/web/frontend/src/api/status.ts`** (+~20 LOC) — extends `rag_provider` type with `provider | model | dim | latency_ms | installed | cached | cache_root | reachable | model_present` fields mirroring the new probe payload shape.
  - **`androscan/web/frontend/src/tabs/SettingsTab.tsx`** (+~25 LOC) — rag_provider StatusCardView call site now passes `extras={[model, "via {provider}", "{latency}ms", "dim {dim}", "cache hit/miss"]}` (fastembed) / `"model present on ollama" / "model NOT present"` (ollama). Card body now actually renders something instead of being blank.
  - **`tests/test_health_probes.py`** (+~210 LOC) — 8 new tests under "ISSUE-024 — warm-singleton functional fastembed probe": functional success path, operator's load-failure path (`NO_SUCHFILE`), not-installed short-circuit, warm-singleton cache hit avoiding re-probe, failure-mode caching, `last_known_embed_provider_ok` pre-probe-is-None / post-success / post-failure. Stand-ins `_FakeFastEmbedProvider` + `_RaisingFastEmbedProvider` keep tests hermetic without standing up real fastembed + ONNX.
  Notable architectural choices: (1) **warm-singleton lives in `health_probes.py`, not in `embed.py`** — keeps the bug fix scoped; the actual query path in `androscan/skills/resolve_ui_element.py` still re-instantiates `FastEmbedProvider` per query (~observed cost; future optimization could share the singleton across probe + query paths, deferred); (2) **process-lifetime cache, no invalidation** — operators swap models by restarting the server; matches LCP.6's grammar-disabled-set cache pattern; (3) **`Optional[bool]` sentinel for cross-reference** — `None` means "no signal yet, leave card as-is"; avoids first-load false-negative on per-app cards before the global probe has fired.
- related tasks/docs:
  - `docs/TASKS.md` (no track designation — see commit message for full as-built record)
  - `androscan/web/health_probes.py::_probe_fastembed_warm` + `::last_known_embed_provider_ok` + `::probe_embed_provider` (the warm-singleton substrate + the public sync/async surfaces)
  - `androscan/web/status_routes.py::_rag_card` (the cross-reference downgrade logic)
  - `androscan/web/frontend/src/tabs/SettingsTab.tsx` (the FE rendering parity fix)
  - `androscan/rag/embed.py::FastEmbedProvider.__init__` (lines 108-139 — the constructor whose ONNX load + dim probe IS the functional probe; this code is unchanged, the probe just now actually uses it)
