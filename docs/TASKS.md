...
|---|----------|------------------|------------|
| 4.1 | **[ ] Smali call-graph backend** | `androscan/analysis/call_graph.py` with v2 fidelity (direct `invoke-*` edges + virtual-dispatch resolution against in-app class hierarchy; `kind: "virtual_dispatch"` tag on inferred edges; `may_have_unresolved_reflection: true` on nodes with reflection patterns). Per-app SQLite store at `apps/<app_id>/.decompiled/<sha>/call_graph.sqlite`. Query API: `GET /api/graph/{app_id}` (paginated/filtered), `GET /api/graph/{app_id}/neighbors/{class.method}`, `GET /api/graph/{app_id}/paths?from=…&to=…`. Auto-build hook on apktool decode completion (mirrors `schedule_rag_build_after_decompile`). New `_call_graph_card` in Settings → Status (build status, freshness, node/edge counts). Fixture-Smali tests for the parser + dispatch resolver + dashed-edge classification. **DEC-016 is amended in the planning commit; this sub-step ratifies it in code.** | apktool output |
| 4.2 | **[ ] Hook Lab UI — Graph pane** | Cytoscape.js graph viewer with strategy **(a) auto-collapse to package level** as the default first-load layout + **(b) right-click "focus subgraph here"** with a hops stepper for method-level focus (skip strategy (c) virtualisation). `virtual_dispatch` edges rendered dashed; `may_have_unresolved_reflection` nodes badged. Click a node → jump into the existing `CodeView` component (Inspect-tab Code Browser) at the right file/line. Search / filter by package or class. | 4.1 |
| 4.3 | **[ ] Frida adapter foundation** | `androscan/adapters/frida_client.py` (attach / detach, hook load, `message` callback into a per-session ring buffer sized by `frida.trace_ring_buffer_size` — default 5000), session lifecycle, frida client/server version probe added to `androscan/web/health_probes.py` and surfaced as a Settings → Status card (operator-managed frida-server, no auto-push in v1). New `pyproject.toml` extra `[frida]` adding `frida`, `frida-tools`, `pyjsparser`. **No UI yet** — this sub-step only stands up the adapter and the readiness signal. | `frida` Python pkg |
| 4.4 | **[ ] Hook template library** | `androscan/adapters/frida_hooks/` with five v1 templates: entry/exit log, SSL-pinning bypass, crypto, SharedPreferences, Intent. Each template ships **both** a JS template and a `pentester_summary_template: str` (Python `.format()`-style with the same parameter names as the JS template). Parametrised renderer + tests for rendering each template (JS + summary). Fail-closed: a template missing its summary template fails the test suite. | 4.3 |
