...
- status: Active *(**v2 complete 2026-04-30; v2.1 follow-up release in progress (ratified 2026-05-05)** — see closing note + v2.1 follow-up release block at end of entry; remains "Active" because (1) the deferred v3 / Phase 12 scope (ISSUE-014, auto-verify, free-form LLM JS, per-app TTL, iOS / WebView / native-binary adapters, Smali patching, LLM-budget real-app measurement follow-up) is still load-bearing for future contributors and is referenced by this DEC's deferral-list rationale, and (2) the v2.1 follow-up release (Trace-mode entry-method discoverability — 7 sub-steps v2.1.0 → v2.1.6) is in progress and extends this DEC's scope without warranting a new DEC per Q10 (α) of the v2.1.0 planning checkpoint (still in v2 family). **Phase 12 — Behavior Trace v3 / cross-tab follow-ups planning checkpoint (12.0)** is **deferred-after-v2.1** (was the next active item per Q2 (C) of the 11.7 planning checkpoint until v2.1 was ratified on 2026-05-05; DEC-026 to be ratified picks the operator-demand-gated v3 candidates from this DEC's deferral list — same workflow shape as the DEC-024 → DEC-025 transition (v1 closing note + v2 follow-up DEC ratified in the same sweep)))*
- date: 2026-04-30
- owners: (project)
- context:
