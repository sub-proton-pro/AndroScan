...
**Status:** **v2 landed 2026-04-30 + v2.1 follow-up release landed 2026-05-05.** **v2:** all eight sub-steps 11.0 → 11.7 complete: 11.0 planning checkpoint + DEC-025 (2026-04-30, docs only); 11.1 Tier 1 (a) decision-timeline UX clarity polish (frontend-only on `DecisionTimeline.tsx` (renamed → `BehaviorTrace.tsx` in Phase 13 sub-step 13.5; legacy file removed at sub-step 13.10) + `App.css`); 11.2 Tier 1 (b) "Trace this method" on call-graph nodes (frontend-only on `CallGraphView.tsx`); 11.3 Tier 1 (c) `BehaviorAnchor`-aware overlay on Manual Hooks Cytoscape pane (new route `GET /api/trace/{app_id}/anchored-methods` + frontend overlay layer + 7 backend tests); 11.4 Tier 2 (d) part 1 — bounded inter-procedural slicer descent + type-driven `is_stateless` analyzer (slicer +~330 LOC over the v1 base + `_DescentBudget` + `_STATELESS_LIB_DENYLIST` + 27 new tests); 11.5 Tier 2 (d) part 2 — same-class field-write-site walking (slicer +~280 LOC + `_walk_field_write_sites` walker + constructor-priority rule + 8 new tests); 11.6 bypass-planner re-run + LLM-tier `trace_behavior` budget revisit + cache schema bump + ISSUE-013 close-out (slicer ~60 LOC delta for `descent_depth` tagging + `trace.sqlite` `SCHEMA_VERSION` 1 → 2 + two new config knobs `ollama_num_ctx` + `trace_max_slice_depth` + LLM client `num_ctx` forwarding + frontend depth-pill UI + 26 new tests); 11.7 docs sweep + DEC-025 closing note (this section's status promotion + the 11.7 row in `docs/TASKS.md` + the parent line + sub-bullet + Recent-completed-work entry + Next-expected-milestone rework in `docs/STATE.md` + DEC-025's v2 closing note in `docs/DECISIONS.md`). Final Phase 11 test count: **+26 new tests** (`tests/test_bypass_planner.py` +6 + `tests/test_decisions_slicing.py` +35 across 11.4 / 11.5 / 11.6 + `tests/test_config.py` +14 + `tests/test_trace_routes.py` +7); near the spec target of ~32 tests. **ISSUE-013 → Resolved (Phase 11 v2)** against the v1-vs-v2 corpus regression-floor measurement test (`tests/test_decisions_slicing.py::test_v1_vs_v2_corpus_measurement_v2_resolves_strictly_more_terminals`); the > 50% production-dogfood threshold from the original DEC-025 framing remains a separate pending verification (re-open if real-app measurement shows v2 still leaves > 50% of v1's None-cases unresolved). **LLM-budget measurement outcome deferred** per Q1 (A) of the 11.7 planning checkpoint — 11.6 shipped the bumped budgets per spec but the actual real-app token-usage measurement was deferred (no dogfood-app traces accrued under v2 within the planning window); follow-up tracked separately when telemetry surfaces, with the existing measure-first-tighten-if-needed posture intact (revert + tighten `_format_decision_for_llm` per-decision prose payload if qwen3.5:35b runtime can't actually use the larger context). See `docs/STATE.md` § Phase 11 sub-bullets for the as-built per-sub-step record + DEC-025's v2 closing note for the v3 hand-off. **v2.1 follow-up release:** all seven sub-steps v2.1.0 → v2.1.6 complete (2026-05-05; mirrors Phase 11's 11.0 → 11.7 cadence): v2.1.0 planning checkpoint + DEC-025 closing-note v2.1 extension (docs only); v2.1.1 deterministic UX backbone — `ClassMethodTree` browser embedded in Trace mode + Smali-field collapse + Hops-inline restructure (FE-only on `LabTraceMode.tsx` + `ClassMethodTree.tsx` + `App.css`); v2.1.2 backend coalescer + frontend debounced spinner + ✓/⚠ validation pill (new `POST /api/trace/{app_id}/normalise-entry` endpoint + +8 backend tests); v2.1.3 Tier 1 "Find similar classes" via `POST /api/trace/{app_id}/suggest-similar-classes` (sibling endpoint using `difflib.SequenceMatcher`; +7 tests); v2.1.4 Tier 2(a) "Ask AI" button + `pendingChatPrefill` plumbing (FE-only); v2.1.5 Tier 3 `androscan/skills/suggest_trace_entry.py` skill + skill-response `widgets[]` schema extension (`SkillResult.widgets: tuple[SkillWidget, ...]` field + `SkillWidget` typed union with `TraceEntryCandidateWidget` as first member) + `<TraceEntryCandidateWidget>` chat renderer + bounded chat agentic loop substrate (`androscan/web/chat.py` refactor with `MAX_AGENTIC_TURNS = 5` / `MAX_SKILLS_PER_TURN = 3` + new `skill_request` / `skill_result` / `skill_pending` / `widget` SSE events; +37 tests; **closes ISSUE-009 as a side-effect** — the chat-widget pattern requires an agentic-loop substrate that DEC-022 had recommended but `androscan/web/chat.py` had been single-pass through Phase 11 v2; v2.1.5 had to ship the loop first as the substrate the widget pattern is built on, an architectural deviation from the v2.1.0 spec which had assumed the loop existed); v2.1.6 docs sweep + DEC-025 closing-note v2.1 extension (docs only). Final v2.1 test count: **+28 new tests** (test count baseline at v2.1.0 was 907; HEAD is 935). **Architectural pattern lock per Q7 (ii) (load-bearing addition):** structured-output extension to skill response schema — first instance of LLM-emitted interactive widgets in chat, deliberately extensible (future skills can reuse the same `widgets[]` schema + render seam; cross-link DEC-022 — the chat-widget pattern extends DEC-022's chat agentic loop with the new `widgets[]` outbound channel + structured-skill-result render seam). DEC-025 stays Active (no new DEC for v2.1 per Q10: α — still in v2 family). **Phase 12 — Behavior Trace v3 / cross-tab follow-ups planning checkpoint (12.0)** is the next active item (was deferred-after-v2.1 between 2026-05-05 v2.1.0 ratification and 2026-05-05 v2.1.6 wrap-up; flips back to Active now that the v2.1 follow-up release is complete); DEC-026 to be ratified picking the operator-demand-gated v3 candidates from DEC-025's deferral list (ISSUE-014, auto-verify, free-form LLM JS, per-app TTL, iOS / WebView / native-binary adapters, Smali patching, LLM-budget real-app measurement follow-up). **Parallel non-blocking thread:** **CI.0 — Phase 4 CI** (pytest on push/PR; long-deferred from Phase 4); originally promoted to Active by DEC-025 in service of the Phase 11 slicer changes but ultimately shipped *after* Phase 11 v2 (the suite passed locally throughout 11.x and across the v2.1 follow-up release, and operator's "Lab engine before CI" direction (2026-04-30) deprioritized it relative to v3 work). Currently deprioritized — promote when Lab engine reaches a stable v3 baseline. Rationale + alternatives + tradeoffs + the four pre-answered planning-checkpoint questions + the v3 deferral list + the v2 closing note + the v2.1 closing-note extension: **DEC-025**.

---

### Phase 13 — Behavior Trace v3 (Lab tab — Dynamic Execution + Flowchart UI overhaul) *(**v1 landed 2026-05-08** — all sub-steps 13.0 → 13.10 complete within a single calendar day; ratified in DEC-029; mirrors the Phase 11 v2.1 / LCP / DEC-028 cadence; see `docs/STATE.md` § "Phase 13 — Behavior Trace v3 — Dynamic Execution + Flowchart UI" sub-bullets for the per-sub-step as-built record + DEC-029's v1 closing note for the planning-checkpoint pre-answers held + canvas-mockup-to-production translation outcome + operator-visible deviations + next-slot operator's call)*

**Goal:** Address the two operator-feedback friction signals from 2026-05-07 → 2026-05-08 dogfooding on top of the v2 / v2.1 static substrate: (1) the existing "Decision Timeline" UI was unintuitive for pentesters who aren't strong developers — the linear list of `DecisionPointCard`s with `true NEUTRAL` / `false NEUTRAL` badges, predicate registers (`v3`, `p0`), and Smali instruction indices (`#11`, `#31`) read as low-level static-analysis output rather than an operator-actionable view; (2) static analysis alone couldn't tell the operator what *actually* fired at runtime — operators kept asking "is this start-to-finish?" / "which methods actually run when I tap Login?", questions only a runtime trace can answer. Phase 13 was ratified ahead of the deferred-pending-telemetry Phase 12 (per operator direction "Ignore phase 12 stuff for now, we'll come to it later after phase 13"); Phase 12's static-precision menu stays deferred-pending-telemetry, untouched by Phase 13.

**Workflow (extended from v1 / v2 / v2.1):** *Anchor → Locate → Trace → Classify → **Visualise → Run dynamic → Inspect** → Manipulate → Verify*. The new "Visualise → Run dynamic → Inspect" loop ships on top of the unchanged Phase 11 v2 / v2.1 static substrate — operators see the call-tree shape via the new `ExecutionFlow` flowchart (top-down DAG; nodes = methods; edges = return-value branches), click a node to open the `Inspector` pane (right-side fixed-360px panel; signature / summary / source line / predicate origin / bypass plans / action row), optionally fire a dynamic trace via the threshold-colored Run button to overlay live execution data (fired methods accent-blue, untaken branches dim/dashed at 55% opacity, live values like `pin="1234" → false` chipped on edges, per-method LLM summaries streaming on first hit). The static workflow shape (anchor + decisions + bypass plans) is unchanged — Phase 13 layers a new visual surface and a new dynamic-execution overlay on top.

**Three coherent deliverables (locked in DEC-029):**

- **UI overhaul.** Rename "Decision Timeline" → "Behavior Trace"; replace the linear timeline with a `ExecutionFlow` flowchart + `Inspector` pane; remove predicate-register / instruction-index / `true NEUTRAL` / `false NEUTRAL` noise from the primary UI (can resurface behind an "Advanced" toggle in a future sub-step if asked). New `androscan/web/frontend/src/components/trace/BehaviorTrace.tsx` (~270 LOC, renamed from `DecisionTimeline.tsx`); new `executionFlowGraph.ts` pure helper (~340 LOC; `BehaviorAnchor` → `{ nodes, edges }` translation with overload-key dedup + BFS layout-rank + synthetic per-source verdict-flavored sinks + `possiblyInlined` heuristic + new `closureMethodCount(anchor)` helper mirroring BE `extract_closure_methods`); new `ExecutionFlow.tsx` (~330 LOC; React Flow / `@xyflow/react` renderer with custom `MethodNode` + `VerdictEdge` enforcing the locked DEC-029 styling — 1.5px stroke + 6×6 arrowhead + color-only edge emphasis + 220×72 cards + 6px radius); new `Inspector.tsx` (~360 LOC; right-side fixed-360px panel with five sections + sticky action-row footer + module-local pure helpers); new `TraceModeToggle.tsx` (~150 LOC; three-pill Static / Dynamic / Both toggle row with `localStorage`-backed per-app persistence). New `~+1300 LOC` of CSS appended to `App.css` covering `.execution-flow-*` / `.inspector-*` / `.trace-mode-toggle-*` / `.lab-dynamic-trace-*` namespaces.

- **Dynamic execution trace.** New `behavior_trace_multi` Frida template (`androscan/adapters/frida_hooks/behavior_trace_multi.py`; ~412 LOC of rendered JS) hooks every method in the active `BehaviorAnchor`'s closure (subject to the threshold-color cap), captures entry/exit events with a per-thread monotonic depth counter (`Map<threadId, Frame[]>` + global `seqCounter` + per-event `parent_call_seq` / `entry_seq` linkage per the 13.1 planning checkpoint's option (b) lock-in), stringifies arguments and return values via a tiered 11-step `summarise(value, depth)` helper (primitives → reflection over user-defined classes via 8 bespoke handlers — null / primitives / String / byte[] / Bundle / Intent / List / Map; locked thresholds 256 chars / 32 bytes / 8 fields / depth 1 / 5 list-or-map entries), and surfaces per-method `Java.use()` failures as `phase: "hook_failed"` events with all three failure reasons (`class_not_found` / `method_not_found` / `impl_set_failed`) for runtime-confirmed `inlined?` detection. New `POST /api/trace/{app_id}/dynamic` REST route + `DELETE /api/trace/{app_id}/dynamic/{session_id}` (in `androscan/web/trace_routes.py`) + new `androscan/web/trace_dynamic.py` pure-helper module (`extract_closure_methods` / `cap_methods` / `methods_to_json`). New `WS /ws/trace/{app_id}/{session_id}` WebSocket multiplexes Frida trace events with three new live-on-first-hit summary event kinds (`summary_pending` / `summary_ready` / `summary_failed`) on the existing `{ts, session_id, kind, payload, raw}` envelope additively per DEC-022 / Phase 11 v2.1.5 discipline. JSONL persistence at `apps/<app_id>/<run>/dynamic_trace.jsonl` (single file per run, parallel to but outside `trace.sqlite`).

- **Per-method LLM summaries.** New `summarise_method` LLM-tier skill (`androscan/skills/summarise_method.py`; ~410 LOC; `requires_confirmation=False`, read-only) takes a method's Smali signature + decompiled source + 4096-byte budget-truncated body + 2-3 lines of context and emits a one-paragraph summary. Cache key = `(app_sha, class_smali, method_name, descriptor)` so summaries persist across runs and across cache schema bumps. Live-on-first-hit (the dynamic-trace WebSocket fires `summary_pending` immediately on first observation; the skill runs in the background; the WS handler emits `summary_ready` with `cached: bool` so the UI distinguishes fresh vs. cached) for methods that fire dynamically OR are explicitly clicked in Inspector. New `androscan/web/trace_summary.py` (~280 LOC) owning the `SummaryCallable` async DI seam + per-method dedup + cache-miss-emits-pending-then-ready / cache-hit-emits-ready-only flow + production adapter `_build_summarise_method_callable` wiring the registered skill behind the seam. The skill is the **second consumer** of the `SkillResult.widgets` channel established by Phase 11 v2.1.5's `<TraceEntryCandidateWidget>` — it emits a new `MethodSummaryWidget` dataclass (registered in `androscan/skills/base.py`'s `SkillWidget` union) on both fresh-LLM and cache-hit paths, rendered by a new `<MethodSummaryWidget>` chat component (~165 LOC) with the same Hook / Trace / Open-source action row as the Inspector + a "Discuss in chat" affordance in the Inspector that pre-fills `pendingChatPrefill` with a method-specific prompt template.

**Locked design (per DEC-029, ratified after a three-round 2026-05-07 → 2026-05-08 plan-mode iteration over the WeakBank `MainActivity.login(…)` example):**
- **Three modes** with one toggle row: `Static` (default before any dynamic run; full DAG, allow/deny sinks colored, candidate gates ringed yellow, suspected-inlined dashed); `Dynamic` (untaken branches dashed at 55% opacity in gray; fired path in accent blue); `Both` (default after first dynamic run; untaken keep verdict color at low opacity; fired path accent blue).
- **Color-only edge emphasis** — same 1.5px stroke + 6×6 arrowhead for all edges. Fired (taken) edges = accent blue, full opacity, solid. Untaken edges in `dynamic` mode = gray, dashed, 55% opacity. Untaken in `both` mode = verdict color (allow=green, deny=red) at 55% opacity, dashed. Static-only mode = verdict color, full opacity, solid. **The plan-mode iteration explicitly tested thicker fired edges and a larger arrowhead and the operator rejected both** ("Blue arrow looks fine, but don't make it thick"); the lock is documented in DEC-029 + `docs/TASKS.md` § Phase 13 risks block to keep it sticky.
- **Threshold-colored Run button** by hook count: ≤ 20 = green, 21-50 = yellow, 51-100 = orange, > 100 = red. Pre-run uses `closureMethodCount(anchor)` (mirrors BE `extract_closure_methods`); post-run uses `state.readyStats.methods_attempted` (the actual BE-attempted count after `hop_cap` truncation).
- **Canonical mockup** at `~/.cursor/projects/Users-sujoy-chakravarti-TW-Work-AI-security-Cursor-AI-AndroScan/canvases/phase-13-trace-mockup.canvas.tsx` — the production frontend visually matched the mockup within the limits of React-on-DOM; three small operator-acknowledged deviations recorded in DEC-029's v1 closing note (action-row 2-column grid; per-thread layout reshape deferred to v2 / ISSUE-019; chat-dock `<MethodSummaryWidget>` integration added on top in 13.9).

**Tests (target was ~+150 → actual was +177):** Spread weighted toward the BE: ~+45 in `tests/test_behavior_trace_multi.py` for the multi-method tracer template + tiered serialiser (13.1), +42 in `tests/test_trace_dynamic.py` (17) + `tests/test_trace_routes.py` (25) for the REST routes + JSONL persistence (13.2), +36 in `tests/test_trace_summary.py` (21) + `tests/test_trace_routes.py` (15 WS-tier) for the WebSocket multiplexing (13.3), +29 in `tests/test_summarise_method_skill.py` for the skill + production adapter (13.4), +8 in `tests/test_summarise_method_skill.py::TestPhase13_9_MethodSummaryWidget` for the chat-widget shape (13.9). Test count baseline at the 13.0 planning checkpoint was 1113 post-DEC-028 / v2.1.8; HEAD at 13.10 is **1290** after Phase 13 BE deltas. Frontend exercised via `tsc --noEmit` + `vite build` per the project convention (no FE test runner in v1).

**Status:** **v1 landed 2026-05-08 + v2.0 landed 2026-05-19 + v2.0-tests next + v2.1 dogfood-gated.** All eleven v1 sub-steps **13.0 → 13.10** complete within a single calendar day. **+177 net new tests** end-to-end (1113 → 1290). **Cumulative bundle delta over 13.5 → 13.10:** CSS +~32 KB raw / +~5 KB gzipped (95.29 KB → 127.16 KB; the +~150 KB JS bump from `@xyflow/react` in 13.6 dominates the JS delta; final 1455.67 KB raw / 454.00 KB gzipped, well within the cumulative bundle ceiling reused across phases). **Zero deviations from DEC-029's three load-bearing locks** (color-only edge emphasis, three-mode toggle, threshold-color hook-count ladder); three small UX accretions ON TOP OF the locks recorded in the v1 closing note. **`BehaviorAnchor` / `DecisionPoint` / `BypassPlan` / `PredicateOrigin` schemas held unchanged** — `trace.sqlite` `SCHEMA_VERSION` stays at `"2"` from Phase 11 v2; cached anchors built under v2 / v2.1 read-and-render under v3 with no migration. Dynamic-trace data lives at the parallel `apps/<app_id>/<run>/dynamic_trace.jsonl` per-run-artifact path (NOT in `trace.sqlite`). **Out-of-scope-for-v1 deferrals captured as Phase 13 v2 candidates** in `docs/KNOWN_ISSUES.md` (ISSUE-017 marching-ants animation; ISSUE-018 branch-outcome inference from dynamic data; ISSUE-019 per-thread layout reshape into thread lanes; ISSUE-020 bootstrap-fetch GET route for cached summaries on Inspector mount of never-fired-this-session methods; ISSUE-021 chat-card "Refresh summary" affordance + pan-to-fit-on-selection — all status `Open (Phase 13 v2 candidate; operator-demand-gated)`). Rationale + alternatives + tradeoffs + locked design history + the v1 closing note covering planning-checkpoint pre-answers held + canvas-mockup-to-production translation outcome + operator-visible deviations + next-slot operator's call: **DEC-029**.

**Phase 13 v2 — Behavior Trace Flowchart UX iteration (ratified in DEC-030; refines DEC-029, does not supersede; two-stage ship vehicle).** Operator dogfooding 2026-05-08 → 2026-05-19 surfaced four operator-visible flowchart UX friction signals on top of the v1 ship: (1) multiple `NEUTRAL` sinks crowding the right edge; (2) bottom-left "fit view" button mis-read as a fullscreen affordance; (3) arrowheads invisible (v1's `fill: currentColor` on shared SVG `<marker>` didn't inherit edge stroke colour); (4) boxes appearing above the Entry method + no indentation in the column-rank layout. **v2.0 (landed 2026-05-19, this commit; DEC-030 ratified Active in same commit)** addresses the four signals with a pure FE change across three files (`executionFlowGraph.ts` + `ExecutionFlow.tsx` + `App.css`, ~+400 LOC net additive; bundle delta +1.83 KB CSS / +2.18 KB JS raw — well within the planning-checkpoint estimate). **Locked Q1-Q8 ratification record (DEC-030):** Q1 = (g) hover-progressive disclosure (small de-emphasized per-source `sink_neutral` always visible at 110×36 with dashed border + 60% opacity + lowercase "neutral" body + sink-hover tooltip listing feeding gates; new per-method-card gate-count badge `${totalGates}g · ${unclassifiedGates}?` rendered always when `totalGates > 0` but CSS-hidden by default + revealed instantly (80ms transition) on `.execution-flow-node:hover` OR `:focus-within` with `-warn` modifier on classifier-uncertainty signal — operator-spottable signal for potential `branch_classifier` false-negatives); Q2 = (a) global ALLOW / DENY sink coalescing via new `GLOBAL_ALLOW_SINK_ID` + `GLOBAL_DENY_SINK_ID` module constants; Q3 = (a) `fill: context-stroke` SVG2 arrowhead fix (one-line CSS change); Q5 = (b) within-column entry-first sort; Q7 = (a) custom within-page fullscreen toggle via React Flow `<Panel position="top-right">` + ESC keydown handler + `-active` class indicator + `aria-pressed` (flips a CSS class on the container expanding it to `position: fixed; inset: 0; z-index: 1000;`); Q4 = (c) defer call-graph caller→callee edge injection to v2.1; Q6 = (b) two-stage ship vehicle (v2.0 lightweight + v2.1 dogfood-gated); Q8 = (b) FE unit tests for `executionFlowGraph.ts` carved into v2.0-tests committed-follow-up sub-step (mid-implementation discovery that no FE test runner exists — carving "introduce frontend test runner" into its own focused ratification surface). **Q1 evolution mid-iteration** — original plan-mode recommendation (a) drop unverdicted/neutral sinks entirely was rejected by operator on false-negative grounds (`branch_classifier` is keyword-based and known to miss real deny gates per `branch_classifier.py:69-83`'s catalog of obfuscated strings / non-English deny copy / custom-predicate gates); operator counter-proposed "small de-emphasized + hover-progressive disclosure" which crystallised as (g) and won verbatim. **Zero deviations from the locked Q1-Q8 design** — every plan-mode answer translated 1-1 to production code. **DEC-029's three load-bearing v1 locks (color-only edge emphasis, three-mode Static/Dynamic/Both toggle, threshold-color hook-count ladder ≤20/21-50/51-100/>100) stay UNTOUCHED by v2.0** — v2 is strictly additive on top of v1, not a replacement of v1. Three new `ExecutionFlowNode` schema fields (`totalGates: number` + `unclassifiedGates: number` + `feedingGates: Array<{ sourceTitle, instructionIndex, verdictKind }>`) populated at graph-build time + new pure helper `gateCountsFor(decisions, key) → { total, unclassified }` + new `ensureSink(sinkKind, sourceId)` factory closure that owns per-kind id-derivation logic — all additive at the type-system layer so pre-existing v1 consumers (`Inspector.tsx`'s `resolveSelection`, `LabTraceMode.tsx`'s `selectedFlowNode` state) read the schema with no changes. **Verification at v2.0 close:** `tsc --noEmit` clean; `vite build` clean (CSS 127.16 → 128.99 KB / +1.83 KB raw, gzipped +0.32 KB; JS 1455.67 → 1457.85 KB / +2.18 KB raw, gzipped +0.71 KB); pytest unchanged at **1290 passed** (BE not touched per the FE-only sub-step pattern from 13.5 / 13.6 / 13.7 / 13.8 / 13.10); zero lint errors across the three touched files. **v2.0-tests (committed follow-up next slot — NOT operator-demand-gated)** lands FE test infrastructure (`vitest` vs `node:test` ratified at v2.0-tests.0 planning checkpoint) + retroactive unit tests for the `executionFlowGraph.ts` pure helper covering entry/gate/method classification, overload coalescing, global ALLOW/DENY sink coalescing, per-source neutral sink emission + `feedingGates` population, unverdicted edge routing, gate-count fields, `layoutRank` assignment, within-column sort. **v2.1 (operator-demand-gated on dogfood signal)** lands call-graph caller→callee edge injection via `androscan/analysis/call_graph.py::neighbors()` + new BE route + FE `kind: "call"` edge variant (thin gray, no arrowhead, no label) so BFS ranks reflect actual call hierarchy + the column-rank layout gains real indentation. **v2.1 gating trigger locked at (a) subjective / dogfood-based** per DEC-030 — operator uses AndroScan with the v2.0 flowchart on real apps; if column-rank layout still feels flat (no clear hierarchy beyond decision flow), v2.1 promotes. No quantitative threshold — friction is the go-signal. **New v2-tracked issue ISSUE-022** opened in `docs/KNOWN_ISSUES.md` 2026-05-19 by DEC-030 ratification for call-graph caller→callee edge injection deferred to v2.1 (dogfood-gated). **Existing ISSUE-017 → ISSUE-021 carried forward into v2 framing** with DEC-030 cross-links added to each; ISSUE-018's discoverability concern partially mitigated by v2.0's gate-count badge with `-warn` modifier surfacing classifier-uncertainty at the source method; ISSUE-021's large-graph navigation concern partially mitigated by v2.0's within-page fullscreen toggle.

**Phase 12** (DEC-026) stays **deferred-pending-telemetry** — the dynamic-trace data Phase 13 ships now changes the calculus on at least two static-precision menu items (ISSUE-014 per-overload precision + LLM-budget real-app measurement follow-up) but Phase 12 promotes back to in-flight Active only when telemetry accrues, regardless of LCP.7 / Phase 13 v2 candidate work. **Next slot post-v2.0-tests is operator's call:** **(1) Phase 13 v2.1** if dogfood surfaces flatness friction; **(2) LCP.7** model-sourcing UX (default per DEC-028 / 13.10 spec); **(3) Phase 12** static-precision menu (telemetry-gated).
...
- **JSON-mode parity baseline (LCP.2) + GBNF / JSON-schema grammar enforcement (LCP.6).** v1 ships `response_format: {"type": "json_object"}` mirroring Ollama's `format: "json"`; LCP.6 (shipped 2026-05-06) added GBNF grammar enforcement for llama.cpp's `grammar:` field on `/v1/chat/completions` + Ollama 0.5.0+ `format: <json-schema>` mode via the new `androscan/llm/grammar.py` module (single emitter sourcing `active_skill_names()` from the live registry, two parallel output shapes — JSON Schema dict for Ollama, hand-rolled GBNF source for llama.cpp). Single commit, no 6a/6b split — the LCP.6 planning checkpoint surveyed `SkillMeta.params_schema` across all 9 LLM-tier skills and confirmed it's operator-prose (not typed JSON Schema), so the discriminated union is on the `skill` enum **only**, not a per-skill discriminated union over heterogeneous `params` shapes. Opportunistic per-base-url 400-fallback caches (`_OLLAMA_SCHEMA_DISABLED_URLS`, `_LLAMACPP_GRAMMAR_DISABLED_URLS`) in `androscan/llm/client.py` so older daemons (Ollama < 0.5.0, `llama-server` builds without `grammar:` field support) transparently fall back to the v1-LCP wire shapes without operator action. New `Config.local_grammar_enabled: bool = True` knob (YAML `llm.local_grammar_enabled` / env `ANDROSCAN_LOCAL_GRAMMAR_ENABLED`, live-reloadable) — kill switch if a runtime / quant pairing produces sampling failures. Closes `ISSUE-016`.
...
**Status:** **Original 7-sub-step backlog complete 2026-05-06; DEC-028 follow-up shipped 2026-05-06; LCP.7 model-sourcing UX planning checkpoint pending.** All sub-steps **LCP.0 → LCP.6** landed within a single calendar day (2026-05-06) across rapid back-to-back per-sub-step Agent-mode sessions; **+157 new tests** end-to-end (test count 935 at LCP.0 baseline → **1092 at LCP.6**; zero regressions in the existing Ollama / cloud paths); cumulative bundle delta JS +~4.1 KB / CSS +2.08 KB (within LCP.0's ceilings; LCP.6 alone added +0.40 KB JS, no CSS change). DEC-027 carries TWO closing notes: the LCP.5 v1-ship note and the LCP.6 final closing note that records the GBNF / JSON-schema enforcement as-built outcome + ISSUE-016 closure. **`ISSUE-016` (JSON-validity drift on aggressive quants Q4_K_M / IQ4_XS) → Resolved (LCP.6)** in `docs/KNOWN_ISSUES.md` — grammar enforcement at the model's logits-sampling level closes the gap; off-vocab skill names + off-range integer fields are impossible at the sampling level. **As-built record:** LCP.0 `52e4f8e` (planning checkpoint + DEC-027 ratification — docs only); LCP.1 `ab174b7` (`LLM_PROVIDERS` table refactor + `Config.provider_kind()` + `is_cloud()` shim + `CLOUD_PROVIDERS` backwards-compat alias — pure refactor, +29 tests); LCP.2 `fc33658` (dedicated `_complete_llamacpp()` HTTP path via `requests` to `/v1/chat/completions` + 3-way switch on `provider_kind()` + `<think>...</think>` strip applied to both local paths + JSON-mode parity baseline — +34 tests in new `tests/test_llm_client.py`); LCP.3 `c21b639` (`probe_llamacpp` health probe + Status panel mutual-exclusion routing keyed on `provider_kind()` + provider-tagged LLM card with `via Ollama` / `via llama.cpp` extras line — +12 tests); LCP.4 `bbe8c95` (Settings UI top-level radio "Local (Ollama)" / "Local (llama.cpp)" / "Cloud" + 3 new `Config.llamacpp_*` fields wired through full plumbing + `Config.active_model` routes on `provider_kind()` + commented `llamacpp:` section in `global_config.yaml` — +21 tests; **single commit, no 4a/4b split** — `SettingsTab.tsx` net add was +251 LOC right at the LCP.0 planning trigger boundary, but a split would have produced an awkward intermediate state where "Local (llama.cpp)" radio shows but the sub-knobs don't render; selective-stage + atomic-commit posture from LCP.1-3 still gives a one-revert rollback story); LCP.5 (docs sweep — DEC-027 LCP.5 v1-ship closing note + ISSUE-016 GBNF deferral entry + README pointer); LCP.6 (this commit — GBNF grammar enforcement: new `androscan/llm/grammar.py` module emitting `build_response_json_schema` for Ollama 0.5.0+ `format: <schema>` mode + hand-rolled `build_response_gbnf` for llama.cpp `grammar:` field on `/v1/chat/completions`; opportunistic per-base-url 400-fallback caches in `androscan/llm/client.py` so older daemons transparently fall back to v1-LCP wire shapes; new `Config.local_grammar_enabled: bool = True` knob — YAML `llm.local_grammar_enabled` / env `ANDROSCAN_LOCAL_GRAMMAR_ENABLED`, live-reloadable, rendered under both "Local (Ollama)" + "Local (llama.cpp)" subsections; +62 tests across new `tests/test_grammar.py` + `tests/test_llm_client.py` + `tests/test_config.py`; **single commit, no 6a/6b split** — the LCP.6 planning checkpoint surveyed `SkillMeta.params_schema` across all 9 LLM-tier skills and confirmed it's operator-prose, not typed JSON Schema, so the LCP.0 emitter-scope estimate was over-scoped ~5x and the actual emitter is small enough for a single commit). **Zero deviations from DEC-027's Q1-Q5 lock-in.** As-built deviations from LCP.0 estimates: LCP.2 +34 vs. estimated +8-10 tests (broader coverage on the new `tests/test_llm_client.py` file); LCP.4 +21 vs. estimated +8 tests (per-field test classes mirror existing patterns like `TestOllamaNumCtx`); LCP.6 +62 vs. estimated +15-20 tests (the per-base-url fallback-cache surface needed end-to-end coverage including dict-mutation-snapshot trap caught by initial test failure). **`ISSUE-016` (JSON-validity drift on aggressive quants) captured at LCP.5, Resolved at LCP.6.** **Q2 (a) lock-in fulfilled at LCP.6** — the same `build_response_json_schema` output works for Ollama 0.5.0+ (Dec 2024 release) AND informs the GBNF derivation for llama.cpp; operators running aggressive quants (Q4_K_M / IQ4_XS) regain reliable structured-JSON output because the model literally cannot emit off-schema tokens at the logits-sampling level. **Branch posture:** shipped on `main` with selective-staging discipline (rejected feature-branch alternative — operator chose the established Phase 10 / 11 / v2.1 sub-step-by-sub-step cadence after the v2.1 follow-up release fully landed and `main` became quiescent at HEAD `15e7c1a`). DEC-027's TWO appended closing notes (LCP.5 + LCP.6) record the full ratification-record × as-built outcome × ISSUE-016-closure block. Sub-step backlog table in `docs/TASKS.md` § **LCP — llama.cpp local provider — sub-step backlog**. Rationale + alternatives + tradeoffs + Q1-Q5 lock-in record + risks: **DEC-027**.

**Post-LCP.6 follow-up — DEC-028 (2026-05-06, single atomic commit).** Operator dogfooding the day after LCP.6 surfaced three orthogonal pieces of feedback that bundled cleanly into one commit: (1) `global_config.yaml` had a disjoint sibling-fanout shape that violated the conceptual unity of the LCP.1 `LLM_PROVIDERS` registry — DEC-028 consolidates `ollama:` / `llamacpp:` / cloud knobs all under a single top-level `llm:` parent; (2) ambiguous `model:` key renamed to `default_model:` in both local provider sub-sections to communicate the "default chat / analysis model unless per-app override fires" semantics that Phase 8's `chat.model_override` made operationally relevant; (3) `llama-server` recommended-startup line crashed with `unknown value for --flash-attn: '--port'` on recent builds because the `-fa` flag now requires an explicit `on` / `off` / `auto` value — fix + cheap-wins (`+ --no-webui` + `+ --cache-reuse 256`) + default-model bumps (`qwen3.6:27b` for Ollama matching the M3 36 GB unified-memory budget; `unsloth/gemma-4-E4B-it-GGUF:Q8_K_XL` for llama.cpp matching the recommended `-hf` arg) + `probe_llamacpp` switched to a two-step `/health` then `/v1/models` flow for cleaner UP / PARTIAL_UP / DOWN classification all rode along. **Hard-break migration** (operator-ratified in single-operator context — Settings UI "Reset to defaults" path writes the new shape in one click). +1 net test (1092 → 1093). See **DEC-028** for the Y1-Y6 lock-in record + as-built notes.

**Post-DEC-028 follow-up — LCP.7 (model-sourcing UX) planning checkpoint pending.** `llama.cpp` model sourcing differs meaningfully from Ollama's single `ollama pull`-managed registry; LCP.7 (operator-agreed 2026-05-06 alongside DEC-028) will surface a 3-way radio "HF cache / HF Hub / Local file" + per-mode input widget in Settings → Global → LLM provider → Local (llama.cpp), with the existing `default_model` text input acting as a free-text fallback for off-vocab labels. Sub-step planning checkpoint pending — no commit lands until the LCP.7.0 Ask-mode planning checkpoint locks the open questions (HF Hub auth UX with optional `HF_TOKEN`; cache-list refresh cadence — `llama-server -cl` is a CLI subcommand, not an HTTP endpoint, so the backend has to shell out or the operator triggers a "Refresh" button; advisory-vs-load-bearing semantic — model selection here sets the `default_model` label for log readability, NOT what `llama-server` actually loaded at startup; file-path picker UX for the M3 local-file mode). See `docs/TASKS.md` § **LCP — llama.cpp local provider — sub-step backlog** LCP.7 row for the spec stub.
