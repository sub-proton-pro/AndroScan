...
**Status:** **v1 ship complete 2026-05-06; LCP.6 GBNF grammar enforcement committed follow-up the only remaining sub-step.** Sub-steps **LCP.0 → LCP.5** all landed within a single calendar day (2026-05-06) across rapid back-to-back per-sub-step Agent-mode sessions; **+95 new tests** end-to-end (test count 935 at LCP.0 baseline → 1030 at LCP.5; zero regressions in the existing Ollama / cloud paths); cumulative bundle delta JS +3.69 KB / CSS +2.08 KB (within LCP.0's ceilings). **As-built record:** LCP.0 `52e4f8e` (planning checkpoint + DEC-027 ratification — docs only); LCP.1 `ab174b7` (`LLM_PROVIDERS` table refactor + `Config.provider_kind()` + `is_cloud()` shim + `CLOUD_PROVIDERS` backwards-compat alias — pure refactor, +29 tests); LCP.2 `fc33658` (dedicated `_complete_llamacpp()` HTTP path via `requests` to `/v1/chat/completions` + 3-way switch on `provider_kind()` + `<think>...</think>` strip applied to both local paths + JSON-mode parity baseline — +34 tests in new `tests/test_llm_client.py`); LCP.3 `c21b639` (`probe_llamacpp` health probe + Status panel mutual-exclusion routing keyed on `provider_kind()` + provider-tagged LLM card with `via Ollama` / `via llama.cpp` extras line — +12 tests); LCP.4 `bbe8c95` (Settings UI top-level radio "Local (Ollama)" / "Local (llama.cpp)" / "Cloud" + 3 new `Config.llamacpp_*` fields wired through full plumbing + `Config.active_model` routes on `provider_kind()` + commented `llamacpp:` section in `global_config.yaml` — +21 tests; **single commit, no 4a/4b split** — `SettingsTab.tsx` net add was +251 LOC right at the LCP.0 planning trigger boundary, but a split would have produced an awkward intermediate state where "Local (llama.cpp)" radio shows but the sub-knobs don't render; selective-stage + atomic-commit posture from LCP.1-3 still gives a one-revert rollback story); LCP.5 (this commit — docs sweep). **Zero deviations from DEC-027's Q1-Q5 lock-in.** As-built deviations from LCP.0 estimates: LCP.2 +34 vs. estimated +8-10 tests (broader coverage on the new `tests/test_llm_client.py` file); LCP.4 +21 vs. estimated +8 tests (per-field test classes mirror existing patterns like `TestOllamaNumCtx`). **JSON-validity drift gap on aggressive quants pre-LCP.6 captured at LCP.5 as ISSUE-016** in `docs/KNOWN_ISSUES.md`. **LCP.6 (committed follow-up per Q2 (a) lock-in)** emits GBNF grammar from the existing JSON schema for `skill_requests` / `hypotheses` — benefits both local providers (Ollama supports JSON-schema mode in newer builds via `format: <json-schema>`); single LCP.6 vs LCP.6a + LCP.6b split decision lives in the LCP.6 planning checkpoint. **Branch posture:** ship on `main` with selective-staging discipline (rejected feature-branch alternative — operator chose the established Phase 10 / 11 / v2.1 sub-step-by-sub-step cadence after the v2.1 follow-up release fully landed and `main` became quiescent at HEAD `15e7c1a`). DEC-027's appended LCP.5 closing note records the full ratification-record × as-built outcome × pending-LCP.6-spec block. Sub-step backlog table in `docs/TASKS.md` § **LCP — llama.cpp local provider — sub-step backlog**. Rationale + alternatives + tradeoffs + Q1-Q5 lock-in record + risks: **DEC-027**.
