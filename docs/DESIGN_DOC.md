...
### Phase 11 — Behavior Trace v2 (Lab tab — bounded inter-procedural slicing + operator-UX polish) *(**v2 landed 2026-04-30** — all sub-steps 11.0 → 11.7 complete; ISSUE-013 → Resolved (Phase 11 v2); see `docs/STATE.md` § "Phase 11 — Behavior Trace v2 (Lab tab — bounded inter-procedural slicing + operator-UX polish)" sub-bullets for the as-built record + DEC-025's v2 closing note for the LLM-budget measurement deferral language)*
