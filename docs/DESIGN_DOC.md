...
### Phase 11 — Behavior Trace v2 (Lab tab — bounded inter-procedural slicing + operator-UX polish) *(planning checkpoint 11.0 done 2026-04-30; **DEC-025** ratified; sub-steps 11.1 → 11.7 pending — see `docs/STATE.md` § "Phase 11 — Behavior Trace v2 (Lab tab — bounded inter-procedural slicing + operator-UX polish)" sub-bullets for as-built status)*
...
**Status:** **active** — 11.0 planning checkpoint + DEC-025 done 2026-04-30 (docs only); 11.1 → 11.7 pending per `docs/TASKS.md` § Phase 11 — Behavior Trace v2 — sub-step backlog. Brief Ask-mode planning checkpoint at the top of 11.4 to confirm the `_DescentBudget` shape and the `_STATELESS_LIB_DENYLIST` initial seed before the slicer code lands (mirrors 10.6's wire-shape checkpoint). **Parallel non-blocking thread:** **CI.0 — Phase 4 CI** (pytest on push/PR; long-deferred from Phase 4); promoted to Active by DEC-025 because the Phase 11 slicer changes are the largest behavioural change of v2 and warrant a regression safety net before shipping. Sequence before 11.4 if scheduling permits. Rationale + alternatives + tradeoffs + the four pre-answered planning-checkpoint questions + the v3 deferral list: **DEC-025**.
