...
def extract_dossier(apk_path: str) -> Dossier:
    ...
    Uses a system temp directory so no persistent files are created at the project root.
    """
    if not (apk_path or "").strip():
        raise ValueError("apk_path must be non-empty")
    config = load_config()
    with tempfile.TemporaryDirectory(prefix="androscan_") as tmp:
        fake_run = Path(tmp) / "run"
        fake_run.mkdir()
