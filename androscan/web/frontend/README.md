...
- `src/tabs/`
  ...
  - `LabTab.tsx` — three-mode workspace (renamed from `HookLabTab.tsx` in Phase 10 sub-step 10.6). A thin left rail switches between **Trace** (UI ➜ decision points ➜ bypass plans, default; the placeholder lives in `LabTraceMode.tsx` with the full `BehaviorAnchorCard` / `DecisionTimeline` / `BypassPlanCard` UI landing in 10.7), **Manual Hooks** (the legacy three-column Hook Lab layout with CallGraph + CodeView + HookBuilder + Sessions), and **Graph** (a dedicated full-pane CallGraphView). Mode selection persists in `localStorage["androscan.lab.mode"]`.
