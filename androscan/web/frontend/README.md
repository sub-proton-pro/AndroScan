...
- `src/tabs/`
  ...
  - `LabTab.tsx` — three-mode workspace (renamed from `HookLabTab.tsx` in Phase 10 sub-step 10.6). A thin left rail switches between **Trace** (UI ➜ Behavior Trace ➜ bypass plans, default; the placeholder lives in `LabTraceMode.tsx` with the full `BehaviorAnchorCard` / `BehaviorTrace` / `BypassPlanCard` UI landing in 10.7 and the section rebranded from "Decision Timeline" → "Behavior Trace" in Phase 13 sub-step 13.5 behind a one-release `VITE_BEHAVIOR_TRACE_LEGACY` rollback flag — the legacy `DecisionTimeline` component stays in the tree until 13.10 sweeps it), **Manual Hooks** (the legacy three-column Hook Lab layout with CallGraph + CodeView + HookBuilder + Sessions), and **Graph** (a dedicated full-pane CallGraphView). Mode selection persists in `localStorage["androscan.lab.mode"]`.
