...
def build_system_content() -> str:
    ...
    return (
        ...
        "\n"
        "CONTENTPROVIDER exploit_params: For ContentProvider hypotheses, use DIFFERENT fields.\n"
        "IMPORTANT: Do NOT construct the full content:// URI. The authority is automatically resolved "
        "from the manifest. Instead provide:\n"
        "  - \"path\": the path after the authority (e.g. \"/account\", \"/users\")\n"
        "  - \"query_params\": URL query parameters without leading '?' (e.g. \"access_token=WB-SECRET-2026\")\n"
        "  - \"operation\": one of \"query\", \"insert\", \"update\", \"delete\", \"read\", \"call\"\n"
        "    Default is \"query\". Use \"read\" for openFile()-based providers (file access).\n"
        "  - \"where\": SQL WHERE clause (e.g. \"1=1\" to dump all rows)\n"
        "  - \"projection\": column projection string\n"
        "  - \"sort\": SQL sort order string\n"
        "  - \"bind\": array of {\"key\": str, \"type\": \"s\"|\"i\"|\"l\"|\"f\"|\"d\", \"value\": str}\n"
        "    for insert/update operations\n"
        "  - \"method\": string for call() operations\n"
        "  - \"arg\": string argument for call() operations\n"
        "Do NOT use action, extras, category, or flags for ContentProvider exploits.\n"
        "ContentProviders use 'content' CLI commands, not 'am' intents.\n"
        "When analyzing a ContentProvider, look for: missing readPermission/writePermission,\n"
        "SQL injection in query/delete/update (string concatenation in selection/projection),\n"
        "path traversal in openFile(), and sensitive data exposure.\n"
        "\n"
        "COMPUTED PARAMS: If exploitation requires a computed hash or signature that you cannot "
        "calculate yourself, add a \"computed_params\" array to exploit_params:\n"
        "  computed_params: [{\"param\": \"sig\", \"algorithm\": \"sha256\", \"input\": \"secret:KEY-2026\"}]\n"
        "Supported algorithms: sha256, sha1, md5, hmac-sha256 (add \"key\" field), base64.\n"
        "The tool will compute the value and append it as a query parameter to the URI automatically.\n"
        "IMPORTANT: Do NOT include the computed parameter in data_uri yourself. For example, if you use "
        "computed_params with param=\"sig\", do NOT put \"sig=...\" or \"sig=<computed>\" in data_uri. "
        "The tool handles injection. Only include other non-computed parameters in data_uri.\n"
        "Do NOT try to compute hash values yourself — they will be wrong. Always use computed_params.\n"
        "Use this for deep links with signature validation, ContentProviders with token checks, etc.\n"
        "\n"
